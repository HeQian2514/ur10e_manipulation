...
if False:
    ...
    from common.RTDE_controller import RTDEInterpolationController
from common.multi_realsense import MultiRealsense, SingleRealsense
...
from common.multi_camera_visualizer import MultiCameraVisualizer
...
DEFAULT_OBS_KEY_MAP = {
    ...
    'GripperPosition': 'robot_gripper_position',
    # timestamps
    'step_idx': 'step_idx',
    'timestamp': 'timestamp'
}
...
class RealEnv:
    ...
    def get_obs(self) -> dict:
        ...
        for camera_idx, value in self.last_realsense_data.items():
            ...
            # camera_obs[f'camera_{camera_idx}_point_cloud'] = value['point_cloud'][this_idxs]
    ...
    def exec_actions(self, 
            ...
            gripper_cmds: Optional[np.ndarray],
        ...
        if not isinstance(gripper_cmds, np.ndarray):
            gripper_cmds = np.array(gripper_cmds, dtype=np.int64)
        ...
        new_gripper_cmds = gripper_cmds[is_new]

        # schedule waypoints
        for i in range(len(new_actions)):
            self.robot.schedule_waypoint(
                pose=new_actions[i],
                gripper=new_gripper_cmds[i],
    ...
    def get_robot_state(self, k=None):
        return self.robot.get_state(k)
